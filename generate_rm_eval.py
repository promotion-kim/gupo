...
def main(args):
    ...
    # ... (나머지 평가 및 저장 로직은 동일) ...
        ...
        output_path = os.path.join(
            args.output_dir,
            f"generation_result_seed{args.seed}.json"
        )
    ...
    for i in range(args.n_samples):
        ...
        if subset:
            subset_mean = float(np.mean([x['reward_score'] for x in subset]))
            index_means.append(subset_mean)
    ...
    std_of_index_means = float(np.std(index_means)) if index_means else 0.0
    ...
    summary_data = {
        ...
        "variation_statistics": {
             ...
             "std_of_index_means": std_of_index_means
        },
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_dir", type=str, help="Path to checkpoint")
