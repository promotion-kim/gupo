...
class AlphaDPOTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        """A trainer for a language model, supporting either AlphaDPO training.
        ...
        self.gamma_beta_ratio = config.loss.gamma_beta_ratio
        self.constant_1 = torch.zeros(1, device=policy.device)
        self.constant_2 = torch.zeros(1, device=policy.device)
        
        if config.loss.name == 'alphadpo':
            self._loss_fn = self.alphadpo_loss
    
    def alphadpo_loss(self,
        ...
        """Compute the AlphaDPO loss for a batch of policy and reference model log probabilities.
        ...
        Returns:
            ...
            The losses tensor contains the AlphaDPO loss for each example in the batch.
        ...
        #!
        eps = 1e-6
        ref_logratios_normalized = (gap_pos_neg - self.constant_1) / torch.clamp(self.constant_2, min=eps)
        ...
        return (losses, 1.0, constant_term), chosen_rewards, rejected_rewards
    ...
    def update_and_sync_ref_mean_std(self, ref_logratios, gamma_update=0.9):
        with torch.no_grad():
            ...
            ref_logratios_global = all_gather_if_needed(ref_logratios_local, self.rank, self.world_size)
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        """Generate samples from the policy and reference model for the given batch of inputs."""
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        """Compute the AlphaDPO loss and other metrics for the given batch of inputs."""

        metrics = {}
        train_test = 'train' if train else 'eval'

        if loss_config.name == 'alphadpo':
        ...
        losses, _, ref_logratios = losses

        all_devices_losses = all_gather_if_needed(losses.detach(), self.rank, self.world_size)
        metrics[f'loss/{train_test}'] = all_devices_losses.cpu().numpy().tolist()

        ref_logratios = all_gather_if_needed(ref_logratios.detach(), self.rank, self.world_size)
        metrics[f'loss/{train_test}_ref_logratios'] = ref_logratios.detach().cpu().numpy().tolist()
        metrics[f'loss/{train_test}_MAX_ref_logratios'] = [ref_logratios.detach().max().item()]
        metrics[f'loss/{train_test}_MIN_ref_logratios'] = [ref_logratios.detach().min().item()]

        return losses.mean(), metrics

    def train(self):
        """Begin either AlphaDPO training, with periodic evaluation."""
