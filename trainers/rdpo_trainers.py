...
class RDPOTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        """A trainer for a language model, supporting either RDPO training.
    ...
    def rdpo_loss(self,
                 ...
                 len_chosen, len_rejected) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the RDPO loss for a batch of policy and reference model log probabilities.
        ...
        Returns:
            ...
            The losses tensor contains the RDPO loss for each example in the batch.
            The chosen_rewards and rejected_rewards tensors contain the rewards for the chosen and rejected responses, respectively.
        """
        logits = self.beta * (policy_chosen_logps - policy_rejected_logps - reference_chosen_logps + reference_rejected_logps)
        gamma = self.alpha * (len_chosen - len_rejected)
        logits = logits - gamma
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        """Compute the RDPO loss and other metrics for the given batch of inputs."""
        ...
        if loss_config.name == 'rdpo':
            ...
            losses, chosen_rewards, rejected_rewards = self._loss_fn(
                ...
                len_chosen=batch['chosen_labels'].shape[0], len_rejected=batch['rejected_labels'].shape[0])
    ...
    def train(self):
        """Begin either RDPO training, with periodic evaluation."""
